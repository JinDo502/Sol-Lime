...
const FAQItem = ({ question, answer }: { question: string; answer: string }) => {
  ...
  return (
    <div className='cursor-pointer' onClick={() => setIsOpen(!isOpen)}>
      <h2 className='flex items-center justify-start py-4 font-semibold gap-4 border-b border-[var(--divider)] border-opacity-10 md:text-xl'>
        {isOpen ? <AiOutlineMinus className='text-2xl' /> : <AiOutlinePlus className='text-2xl' />}
        {question}
      </h2>

...
// 单个特性项组件
const FeatureItem = React.memo(({ item, index }: { item: (typeof featuresContent)[0]; index: number }) => {
  return (
    <motion.div className='col' variants={fadeInUp}>
...
// 详细特性项组件
const DetailedFeatureItem = React.memo(({ item, index, controls }: { item: (typeof featuresContent2)[0]; index: number; controls: LegacyAnimationControls }) => {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div className='w-full md:1/2 lg:w-1/3 flex justify-end' whileHover={{ scale: 1.05 }} transition={springs.soft}>
        <Image src={item.img} alt={item.title} width={500} height={500} className='w-full' sizes='(max-width: 768px) 100vw, 33vw' loading='lazy' />
      </motion.div>
    </motion.div>
  );
});

DetailedFeatureItem.displayName = 'DetailedFeatureItem';
...
const Features = () => {
  ...
  const featuresItems = useMemo(() => featuresContent.map((item, index) => <FeatureItem key={item.title} item={item} index={index} />), [featuresContent]);

  return (
    <>
      <section className='relative overflow-hidden py-10 px-6'>
        <div className='absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] z-[-1]'>
          <Image src='/images/shapes/blurry-shape-3.svg' alt='' width={1000} height={1000} sizes='100vw' priority={false} loading='lazy' />
        </div>
      ...
      <section className='py-10 px-6'>
        <div className='container mx-auto grid grid-cols-1 gap-8'>
          <div ref={ref1}>
            <DetailedFeatureItem item={featuresContent2[0]} index={0} controls={controls1} />
          </div>
          <div ref={ref2}>
            {featuresContent2.slice(1).map((item, index) => (
              <DetailedFeatureItem key={item.title} item={item} index={index + 1} controls={controls2} />
            ))}
          </div>
        </div>
      </section>

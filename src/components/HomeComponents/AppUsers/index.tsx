...
const AppUsers = () => {
  return (
    <section className='py-10 px-6 bg-card-bg'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-10 pt-15 md:gap-0 md:divide-x-1 md:divide-divider'>
        {appUsersContent.map((item) => {
          return (
            <div key={item?.title} className='flex flex-col items-start md:items-center gap-4 md:px-5'>
              <div className='w-14 h-14 text-background bg-primary rounded-2xl p-2'>{item.icon}</div>
      ...
      <div className='hidden md:flex justify-between mt-10'>
        {[0, 1].map((_, index) => (
          <div key={index} className='h-[1px] bg-divider opacity-50' style={{ width: '32%' }} />
        ))}
      </div>
    </section>
  );
